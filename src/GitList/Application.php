...
class Application extends FrameworkApplication
{
    ...
    public function __construct(Config $config, $root = null)
    {
        ...
        $this['treegraph_limit'] = $config->get('app', 'treegraph_limit') !== false ? (int)$config->get('app', 'treegraph_limit') : 800;
        ...
        $this['twig'] = $this->extend('twig', function ($twig, $app) use ($pkg, $config, $enable_editing) {
            ...
            $twig->addFilter(new TwigFilter('format_date_iso', array($app, 'formatDateIso')));
            ...
            $rawLangCookie = isset($_COOKIE['p3x-gitlist-language']) ? $_COOKIE['p3x-gitlist-language'] : 'auto';
            $isAutoLang = ($rawLangCookie === 'auto');
            ...
            if ($isAutoLang) {
                ...
                if ($acceptLang !== '') {
                    // Parse Accept-Language, e.g. "hu-HU,hu;q=0.9,en-US;q=0.8,en;q=0.7"
                    $parts = explode(',', $acceptLang);
                    foreach ($parts as $part) {
                        $langPart = explode(';', trim($part))[0];
                        $langCode = strtolower(explode('-', trim($langPart))[0]);
            ...
            $twig->addGlobal('is_auto_lang', $isAutoLang);
            $twig->addGlobal('allowed_langs', $allowedLangs);
            $twig->addGlobal('lang_names', $langNames);

            $currentTheme = !isset($_COOKIE['gitlist-bootstrap-theme']) ? 'auto' : $_COOKIE['gitlist-bootstrap-theme'];
            $isAutoTheme = ($currentTheme === 'auto');
            if ($isAutoTheme) {
                $currentTheme = 'bootstrap-cosmo';
            }
            $themeDark = [
                'cyborg',
                'darkly',
            ...
            $twig->addGlobal('is_auto_theme', $isAutoTheme);

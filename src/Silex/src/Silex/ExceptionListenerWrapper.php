...
class ExceptionListenerWrapper
{
    ...
    protected function shouldRun(\Exception $exception)
    {
        ...
        if ($callbackReflection->getNumberOfParameters() > 0) {
            ...
            $class = $expectedException->getType() && !$expectedException->getType()->isBuiltin() ? new \ReflectionClass($expectedException->getType()->getName()) : null;

            if ($class && !$class->isInstance($exception)) {
                return false;
            }
        }
